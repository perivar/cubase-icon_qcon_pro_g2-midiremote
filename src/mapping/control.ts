...
const setShiftableButtonsLedValues = (
  ...
  for (
    ...
    // TODO: this does not seem to work, only the Left button lights up
...
export const bindControlButtons = (
  ...
  for (let i = 0, arr = buttons.function; i < arr.length; i++) {
    const button = arr[i];
...
export const bindFootControl = (
  ...
  // PIN: converted for-of loop to ES5
  for (let i = 0, arr = controlSectionElements.footSwitches; i < arr.length; i++) {
    const footSwitch = arr[i];
